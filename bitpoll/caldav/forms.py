...
class URLAuthWidget(MultiWidget):
    ...
    def __init__(self):
        widgets = (URLInput(), TextInput(), PasswordInput())
...
class URLAuthField(MultiValueField):
    ...
    def __init__(self, **kwargs):
        max_lengh = kwargs.pop(
            "max_length"
        )  # TODO use this in validation of the whole field
        required = kwargs.pop(
            "required"
        )  # TODO use this in validation of the whole field
        fields = (
...
class DavCalendarForm(ModelForm):
    ...
    class Meta:
        ...
        field_classes = {"url": URLAuthField}

    def clean(self):
        cleaned_data = super(DavCalendarForm, self).clean()
        bucket = TokenBucket.get(
            identifier="calendars_added",
