...
def change_calendar(request, calendar_id: int = None):
    ...
    if form.is_valid():
        ...
        old_calendars = DavCalendar.objects.filter(user=request.user).exclude(
            id=calendar_id
        )
        exists = False
        ...
        if not exists:
            try:
                calendar = Calendar(
                    client=DAVClient(caldav_obj.url), url=caldav_obj.url
                )
                ...
                form.add_error(
                    None, ugettext_lazy("Error getting Calendar: {}".format(str(e)))
                )
        else:
            form.add_error(None, ugettext_lazy("This calendar is already configured"))
    elif "delete" in request.POST:
        obj = get_object_or_404(
            DavCalendar, id=request.POST["delete"], user=request.user
        )
        obj.delete()
        return redirect("settings")
