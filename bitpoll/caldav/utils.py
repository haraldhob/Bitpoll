...
def get_caldav(choices: List[Choice], current_poll: Poll, user: BitpollUser, request):
    # calendar stuff
    events2 = []
    if not settings.CALENDAR_ENABLED or not (
        user.is_authenticated and current_poll.type == "datetime" and choices
    ):
    ...
    for calendar_obj in user.davcalendar_set.all():
        cache_key = "calendar_{}_events_{}-{}".format(
            calendar_obj.id, start, end
        ).replace(" ", "_")
        ...
        if events_calendar is None:
            ...
            try:
                calendar = DavCalendar(
                    client=DAVClient(calendar_obj.url), url=calendar_obj.url
                )
                            ...
                            try:
                                if "DTEND" in event:
                                    ...
                                    if isinstance(duration, list):
                                        duration = duration[
                                            0
                                        ]  # todo: use all elements?? what does it mean if there are more than one element?
