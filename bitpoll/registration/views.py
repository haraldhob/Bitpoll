...
from bitpoll.registration.forms import RegisterForm, NickChangeForm, EmailChangeForm
...
def request_successful(request, email):
    return TemplateResponse(
        request, "registration/request_successful.html", {"email": email}
    )
...
def _verify_email(request, email):
    ...
    return _send_mail_or_error_page(
        _("Verify this address for %s" % settings.SITE_NAME),
        email_content,
        email,
        request,
    )
...
def _finish_account_request(request, info):
    ...
    email_content = render_to_string(
        "registration/create_email.txt", {"activation_link": activation_link}
    )
    return _send_mail_or_error_page(
        _("Account creation at %s" % settings.SITE_NAME), email_content, email, request
    )
...
def _send_mail_or_error_page(subject, content, address, request):
    try:
        ...
        if not unknown:
            error_email_content = "{0}: {1}".format(
                e.__class__.__name__, repr(e.recipients)
            )
            send_mail(
