...
class Invitation(models.Model):
    ...
    creator = models.ForeignKey(
        BitpollUser, on_delete=models.CASCADE, related_name="creator"
    )
    ...
    def send(self, request):
        if (not self.last_email) or self.last_email + timedelta(
            hours=12
        ) < now():  # TODO: TIMEDELTA mit config
            old_lang = translation.get_language()
            translation.activate(self.user.language)
            link = reverse(
                "poll_vote", args=(self.poll.url,)
            )  # TODO: hier direkt das poll oder das Vote?
            email_content = render_to_string(
                "invitations/mail_invite.txt",
                {
                    "receiver": self.user.displayname
                    or self.user.get_full_name()
                    or self.user.username,
                    "creator": self.creator.displayname
                    or self.creator.get_full_name()
                    or self.creator.username,
                    "link": link,
                },
            )
            try:
                send_mail(
                    translation.gettext("Invitation to vote on ")
                    + "{}".format(self.poll.title),
                    email_content,
                    None,
                    [self.user.email],
                )
            ...
            messages.error(
                request,
                _(
                    "You have send an Email for {} in the last 12 Hours".format(
                        self.user.username
                    )
                ),
            )
