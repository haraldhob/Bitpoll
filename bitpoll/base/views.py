...
def index(request):
    ...
    """
    public_polls = Poll.objects.filter(
        public_listening=True
    )  # TODO: limit & sotierung & pagination oder so?
    randomize_url = (
        settings.DEFAULT_RANDOM_SLUG == "true"
    )  # this settings value is a javascript true/false
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            ChoiceValue(
                title="yes", icon="check", color="90db46", weight=1, poll=current_poll
            ).save()
            ChoiceValue(
                title="no", icon="ban", color="c43131", weight=0, poll=current_poll
            ).save()
            ChoiceValue(
                title="maybe",
                icon="question",
            ...
            ChoiceValue(
                title="Only if absolutely necessary",
                icon="thumbs-down",
                color="B0E",
                weight=0.25,
                poll=current_poll,
            ...
            if (
                current_poll.type == "universal"
            ):  # TODO: heir könnte auch auf die algemeine edit url weitergeleitet werden
