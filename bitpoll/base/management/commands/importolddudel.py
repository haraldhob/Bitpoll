    ...
    def handle(self, *args, **options):
        ...
        for poll in cursor:
            ...
            migrated_poll = Poll(
                ...
                due_date=_fix_timezone(poll["due_date"], timezone)
        ...
        print(
            "migrating comments, choice values, choices and votes for previously imported polls"
        )
        count = 0
        ...
        for poll_id, poll in migrated_polls:
            ...
            # choices
            cursor.execute(
                "SELECT * FROM choice WHERE deleted=false AND poll_id=%s ORDER BY date, text;",
                (poll_id,),
            )
            sort_key = 0
            for choice in cursor:
                ...
                c = Choice.objects.create(
                    text=choice["text"],
                    date=_fix_timezone(choice["date"], timezone)
                    if choice["date"]
                    else None,
                    poll=poll,
                    sort_key=sort_key,
            ...
            # choice values
            cursor.execute(
                "SELECT * FROM choice_value WHERE deleted=false AND poll_id=%s;",
                (poll_id,),
            )
            ...
            for vote in cursor:
                ...
                v = Vote.objects.create(
                    ...
                    assigned_by=self.resolve_user_or_group(vote["assigned_by_id"])
                    if vote["assigned_by_id"]
                    else None,
                    poll=poll,
                    user=self.resolve_user_or_group(vote["user_id"])
                    if vote["user_id"]
                    else None,
                )
            ...
            for vote_id, vote in votes_migrated:
                # vote choices
                cursor.execute(
                    "SELECT * FROM vote_choice WHERE vote_id=%s;", (vote_id,)
                )
                for vote_choice in cursor:
                    ...
                    try:
                        VoteChoice.objects.create(
                            ...
                            value=choice_values[vote_choice["value_id"]]
                            if vote_choice["value_id"]
                            else None,
                            vote=vote,
                            choice=choices[vote_choice["choice_id"]]
                            if vote_choice["choice_id"]
                            else None,
                        )
            ...
            # comments
            cursor.execute(
                "SELECT * FROM comment WHERE poll_id=%s AND deleted=false;", (poll_id,)
            )
