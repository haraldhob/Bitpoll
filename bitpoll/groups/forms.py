...
class InvitationForm(forms.Form):
    ...
    def clean_invitees(self):
        ...
        for invitee in invitees:
            ...
            has_invitation = bool(
                GroupInvitation.objects.filter(group=self.group, invitee=invitee_user)
            )
            if has_invitation:
            ...
            if already_member:
                raise forms.ValidationError(
                    _('"%s" is already a member of this group.') % invitee
                )
    ...
    def get_invitations(self):
        ...
        for invitee_user in self.invitee_users:
            invitations.append(
                GroupInvitation(
                    group=self.group, invitee=invitee_user, invited_by=self.user
                )
            )
        return invitations
