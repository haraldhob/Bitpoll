...
def show(request, group_name):
    ...
    if not properties.public_members:
        can_see = group.user_set.filter(
            pk=request.user.pk
        ) or GroupInvitation.objects.filter(group=group, invitee=request.user)
        if not can_see:
...
def invite(request, group_name):
    ...
    return TemplateResponse(
        request, "groups/invite.html", {"group": group, "form": form}
    )
...
def _send_invitation_mail(request, invitation, subject, template_name):
    ...
    message = template.render(
        {"invitation": invitation, "site": get_current_site(request)}
    )
